...
#define HSI_CLOCK_SPEED 8000000    // Valor para la señal principal del reloj (HSI -> High Speed Internal)
#define HSE_CLOCK_SPEED 16000000   // Valor para la señal principal del reloj (HSE -> High Speed External)
...
/*
 * Y ahora debemos hacer lo mismo pero cada una de las posiciones de memoria de los periféricos descritos en la Tabla 1 del manual de referencia del micro.
 * Observe que en dicha tabla está a su vez dividida en cuatro segmentos, cada uno correspondiente
 * a APB1, APB2, AHB1, AHB2.
 *
 * Comenzar de arriba hacia abajo como se muestra en la tabla. Inicia USB_OTG_FS_BASE_ADDR.
 */
...
//#define SPI5_BASE_ADDR    (APB2_BASE_ADDR + 0x3000U)
//#define SDIO_BASE_ADDR    (APB2_BASE_ADDR + 0x2C00U)
//#define ADC1_BASE_ADDR    (APB2_BASE_ADDR + 0x1400U)
...
//#define I2C2_BASE_ADDR    (APB1_BASE_ADDR + 0x5400U)
//#define I2C1_BASE_ADDR    (APB1_BASE_ADDR + 0x5000U)
//#define USART2_BASE_ADDR  (APB1_BASE_ADDR + 0x4400U)
//#define I2S2ext_BASE_ADDR (APB1_BASE_ADDR + 0x4000U)
 ...
 * Debido a los temas que se van a manejar en el curso, solo se deben definir los bits de los registros 6.3.1 (RCC_CR)
 * hasta el 6.3.12 (RCC_APB2ENR), 6.3.17 (RCC_BDCR) y 6.3.18 (RCC_CSR).
 *
 * NOTA: La posición de memoria (offset) debe encajar perfectamente con la posición de memoria en la hoja de datos del equipo.
 * Observe que los elementos "reservedx" también están presentes.
 */
...
/*
 * Hacemos como un "merge", en el cual ubicamos la estructura RCC RegDef_t a apuntar a la posición de memoria correspondiente,
 * de forma que cada variable dentro de la estructura coincide con cada uno de los SFR en la memoria del micro-controlador.
 */
 ...
 * Microncontroller prescaler Clock Output2 (MCO2PRE) options
 ...
 * de cada uno de los registros que componen el dicho periférico, pero sí es necesario comprender qué hace cada registro,
 * para poder cargar correctamente la configuración.
 */
